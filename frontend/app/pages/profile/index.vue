...
<template>
  <div class="max-w-md mx-auto space-y-4">
    ...
    <!-- Переключатель роли -->
    <div class="bg-white rounded-xl shadow-sm p-5 space-y-4">
      <h2 class="font-medium text-gray-900">Режим работы</h2>

      <div class="flex gap-2">
        <button
          v-for="r in [['customer','Покупатель'], ['driver','Курьер'], ['manager','Ресторан']]"
          :key="r[0]"
          @click="selectedRole = r[0] as any"
          class="flex-1 py-2 rounded-lg text-sm font-medium border transition-colors"
          :class="selectedRole === r[0]
      ...
      <button
        @click="saveRole"
        class="w-full border border-brand-500 text-brand-600 font-medium py-2 rounded-lg hover:bg-brand-50 transition-colors"
      >
        Применить режим
      </button>
    </div>
